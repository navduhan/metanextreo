...
def rename_fasta_headers(input_fasta, output_fasta):
    ...
    with open(input_fasta, "r") as infile, open(output_fasta, "w") as outfile:
        ...
        for num, record in enumerate(SeqIO.parse(infile, "fasta"), start=1):
            # Rename the header to "metanextreo_contigs_<num>"
            record.id = f"metanextreo_contigs_{num}_{len(record.seq)}"
...
def main():
    ...
    parser.add_argument(
        ...
        "--output",
        required=True,
        help="Path to save the output FASTA file with renamed headers",
    )
